...
if [[ ! -f "$PLUGIN_ROOT/shojaei-seo-for-woo.php" ]]; then
  echo "[damavand] missing plugin at $PLUGIN_ROOT/shojaei-seo-for-woo.php" >&2
  exit 1
fi
...
if [[ ${#php_files[@]} -lt 1 ]]; then
  echo "[damavand] no PHP files found" >&2
  exit 1
fi
fail=0
...
function map_currency(string $currency): array {
    if ($currency === 'IRT') return ['IRR', 10.0];
    return [$currency, 1.0];
}
[$c, $f] = map_currency('IRT');
assert($c === 'IRR' && $f === 10.0);
