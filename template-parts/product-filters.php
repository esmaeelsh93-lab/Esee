...
$filter_data    = parisacrop_get_archive_filter_data();
...
$archive_url    = $queried_object instanceof WP_Term ? get_term_link( $queried_object ) : parisacrop_shop_url();
$archive_url    = is_wp_error( $archive_url ) ? parisacrop_shop_url() : $archive_url;
...
<details class="archive-filters" data-archive-filters <?php echo $has_selection ? 'open' : ''; ?>>
	<summary>
		<span>
			...
			<?php esc_html_e( 'فیلتر محصولات', 'parisacrop' ); ?>
		</span>
		<small><?php echo $has_selection ? esc_html__( 'فیلتر فعال', 'parisacrop' ) : esc_html__( 'سایز و قیمت', 'parisacrop' ); ?></small>
	...
	<form class="archive-filters__form" method="get" action="<?php echo esc_url( $archive_url ); ?>">
		<?php if ( $filter_data['sizes'] ) : ?>
			<fieldset class="archive-filters__sizes">
				<legend><?php esc_html_e( 'سایزهای موجود', 'parisacrop' ); ?></legend>
				<div>
		...
		<?php if ( $has_price ) : ?>
			<fieldset class="archive-filters__price" data-price-range>
				<legend><?php esc_html_e( 'محدوده قیمت', 'parisacrop' ); ?></legend>
				<div class="archive-filters__price-inputs">
					<label>
						<span><?php esc_html_e( 'از', 'parisacrop' ); ?></span>
					...
					<label>
						<span><?php esc_html_e( 'تا', 'parisacrop' ); ?></span>
				...
				>
					<input type="range" min="<?php echo esc_attr( $filter_data['min_price'] ); ?>" max="<?php echo esc_attr( $filter_data['max_price'] ); ?>" step="<?php echo esc_attr( $filter_data['step'] ); ?>" value="<?php echo esc_attr( $active_min ); ?>" aria-label="<?php esc_attr_e( 'کمترین قیمت', 'parisacrop' ); ?>" data-price-min-range>
					<input type="range" min="<?php echo esc_attr( $filter_data['min_price'] ); ?>" max="<?php echo esc_attr( $filter_data['max_price'] ); ?>" step="<?php echo esc_attr( $filter_data['step'] ); ?>" value="<?php echo esc_attr( $active_max ); ?>" aria-label="<?php esc_attr_e( 'بیشترین قیمت', 'parisacrop' ); ?>" data-price-max-range>
				</div>
				<p>
		...
		<div class="archive-filters__actions">
			<button type="submit"><?php esc_html_e( 'اعمال فیلتر', 'parisacrop' ); ?></button>
			<?php if ( $has_selection ) : ?>
				<a href="<?php echo esc_url( $archive_url ); ?>"><?php esc_html_e( 'پاک‌کردن', 'parisacrop' ); ?></a>
			<?php endif; ?>
		</div>
	</form>
</details>
