/**
 * Damavand AI — OpenRouter free models: Alt + related keywords only.
 */
(function ($) {
	'use strict';
	...
	var allowed = damavandAI.allowedKinds || ['related_keywords', 'alt_texts'];
	...
	function applyResult(kind, d) {
		if (kind === 'related_keywords' && d && d.related) {
			$('#dm-score-related').val(d.related).trigger('input');
	...
	function generate(kind) {
		if (allowed.indexOf(kind) === -1) {
			setStatus('این قابلیت AI غیرفعال است.', true);
			return;
		}
		...
		$.ajax({
			...
			type: 'POST',
			data: payload,
			timeout: kind === 'alt_texts' ? 240000 : 60000
		}).done(function (res) {
			if (!res || !res.success) {
			...
			var data = res.data && res.data.data !== undefined ? res.data.data : res.data;
			applyResult(kind, data);
		}).fail(function (xhr) {
			setStatus(ajaxErrorMessage(xhr, damavandAI.i18n.error), true);
		}).always(function () {
			setBusy(false);
		});
	}
	...
	function rebuildModelSelect(preferred) {
		var $sel = $('#shojaei_seo_ai_model');
		if (!$sel.length || !damavandAI.modelPresets) {
			return;
		}
		var rows = damavandAI.modelPresets.openrouter || [];
	...
	$(function () {
		...
		if ($('#shojaei_seo_ai_model').length) {
			rebuildModelSelect();
		}
