...
class AAW_Jalali {
	 ...
	 * @param string $format 'full' برای «۱۶ مرداد ۱۴۰۴»، 'short' برای «۱۴۰۴/۰۵/۱۶» یا 'time' برای افزودن ساعت.
	 */
	...
	/**
	 * فرمت کردن رشته‌ی تاریخ‌زمان میلادی (Y-m-d H:i:s) به رشته‌ی شمسی همراه ساعت.
	 */
	public static function format_datetime( $datetime_string ) {
		$parts = explode( ' ', trim( $datetime_string ) );
		$date  = self::format( $parts[0], 'short' );
		$time  = isset( $parts[1] ) ? substr( $parts[1], 0, 5 ) : '';

		return trim( $date . ' - ' . self::to_persian_digits( $time ) );
	}
	...
	/**
	 * فرمت‌بندی مبلغ پول به همراه واحد پول ووکامرس (بدون تغییر HTML؛ برای متن ساده).
	 */
